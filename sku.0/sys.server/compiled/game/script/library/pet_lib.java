...
//        sendSystemMessageTestingOnly(master, "TAMEDEBUG: makePet EXIT pet=" + pet + " exists=" + exists(pet));
    ...
    {
        // Pre-CU: creature pets use COMBAT_PET; droids use active-droid list + command modules.
    ...
    /**
     * Extra droid slots granted by a single PCD/deed command module.
     * Values 1-5 are direct extra slots. Values 6-100 are treated as craft quality
     * and map to 1-5 extras ((q+19)/20).
     */
    public static int getDroidCommandExtraSlots(obj_id pcdOrDeed) throws InterruptedException
    {
        ...
        {
            // Craft quality 1-100 → 1-5 extra slots
    ...
    /** Max droids that may be out: 1 + sum of command extras on currently OUT droids. */
    ...
    public static void addActiveDroid(obj_id master, obj_id pet) throws InterruptedException
    {
        ...
        // Keep COMBAT_OTHER pointing at one valid droid for legacy code
    ...
    public static void removeActiveDroid(obj_id master, obj_id pet) throws InterruptedException
    {
        ...
        // If packing reduced command capacity, store excess droids
    ...
    public static void enforceDroidCommandCap(obj_id master) throws InterruptedException
    {
        ...
        while (v.size() > max)
        {
            ...
            // storePet triggers removeActiveDroid; refresh
            v = getActiveDroidVector(master);
            if (v.size() > 0 && !isIdValid(excess))
            {
                break;
            }
            // safety: remove last if still present
            if (v.size() > max && v.size() > 0)
            {
                v.remove(v.size() - 1);
                utils.setScriptVar(master, SCRIPTVAR_ACTIVE_DROIDS, v);
            }
...
//        sendSystemMessageTestingOnly(master, "TAMEDEBUG: storePet about to messageTo handlePackRequest pet=" + pet);
    ...
    public static void addToPetList(obj_id master, obj_id pet) throws InterruptedException
    {
        ...
        if (ctype == callable.CALLABLE_TYPE_COMBAT_OTHER || isDroidPet(pet))
        {
        ...
        if (alreadyHas && isIdValid(existing) && existing != pet)
        {
